...
(**
Este trabalho apresenta uma prova formal da correção do algoritmo de ordenação por borbulhamento (a função [bs] a seguir). 
 A formalização foi feita no assistente de provas Coq. 
 O assistente de provas Coq utiliza o sistema de Dedução Natural, 
 o que o torna adequado para o desenvolvimento de atividades computacionais no curso de Lógica Computacional 1. 
 O Coq permite a extração de código certificado em diversas linguagens funcionais, como Ocaml, Haskell e Scheme. 
*)

(** Iniciaremos definindo a função [bubble] que recebe uma lista de naturais como argumento, 
e percorre esta lista comparando elementos consecutivos. 
Chamamos este processo de borbulhamento: 
*)
...
(** Observe que esta função não é estruturalmente recursiva porque, 
xpor exemplo, a lista [(x::l)] não é uma sublista da lista original [(x::y::l)]. 
Neste caso, utilizamos [Function] para construir esta função e precisamos fornecer a medida que decresce em cada chamada recursiva, 
além de provar que esta medida efetivamente decresce a cada chamada recursiva. P
or exemplo, [bubble (2::1::nil)] retorna a lista [(1::2::nil)].
 *)
...
(** A função principal, ou seja, o algoritmo bubble sort propriamente dito, 
é dada pela função [bs] abaixo que recebe uma lista de naturais como argumento:
*)
...
(** Sabemos que aplicar a função [bubble] a uma lista qualquer, 
não necessariamente vai retornar uma lista ordenada, 
...
Proof.
  intros l.
  functional induction (bubble l).
  - (* Caso 1: [] — bubble [] = [], mesma lista *)
    reflexivity.
  - (* Caso 2: [x] — bubble [x] = [x], mesma lista *)
    reflexivity.
  - (* Caso 3: x <= y, sem troca. Ex: [1,2,3] *)
    (* bubble [1,2,3] = 1 :: bubble [2,3] *)
    (* Queremos: [1,2,3] ~ 1 :: bubble [2,3] *)
    (* perm_skip: basta provar [2,3] ~ bubble [2,3] — vem da IHl0 *)
    apply perm_skip. assumption.
  - (* Caso 4: x > y, com troca. Ex: [2,1] *)
    (* bubble [2,1] = 1 :: bubble [2] = [1,2] *)
    (* Queremos: [2,1] ~ [1,2] *)
    (* passo 1: [2,1] ~ [1,2]  (troca com perm_swap) *)
    (* passo 2: [1,2] ~ 1 :: bubble [2] = [1,2]  (perm_skip + IHl0) *)
...
Proof.
  ...
  - (* Caso 1: [] — bs [] = [] *)
    simpl. reflexivity.
  - (* Caso 2: a :: l. Ex: a=3, l=[2,1] *)
    (* bs [3,2,1] = bubble (3 :: bs [2,1]) = bubble [3,1,2] *)
    (* Queremos: [3,2,1] ~ bubble [3,1,2] = [1,2,3] *)
    simpl.
    (* perm_trans com lista do meio [3,1,2] (= a :: bs l): *)
    (*   passo 1: [3,2,1] ~ [3,1,2]  — perm_skip + IH (cauda [2,1] ~ [1,2]) *)
    (*   passo 2: [3,1,2] ~ bubble [3,1,2]  — bubble_perm *)
...
Proof.
  intros l.
  (* Ex: l = [3,2,1]. split separa em duas metas: *)
  (*   1) bs [3,2,1] está ordenado  →  [1,2,3]  (bs_sorted) *)
  (*   2) [3,2,1] ~ bs [3,2,1]      →  [3,2,1] ~ [1,2,3]  (bs_permuta) *)
  split.
  - apply bs_sorted.
  - apply bs_permuta.
Qed.  
